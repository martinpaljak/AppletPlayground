 ...
 */
public class OpenPGPApplet extends Applet implements ExtendedLength {
	...
	private static short BUFFER_MAX_LENGTH = 512;
	...
	private void commandChaining(APDU apdu) {
		byte[] buf = apdu.getBuffer();
		short p1p2 = Util.makeShort(buf[ISO7816.OFFSET_P1],
		...
		if ((byte) (buf[ISO7816.OFFSET_CLA] & (byte) 0x10) == (byte) 0x10) {
			// If chaining was already initiated, INS and P1P2 should match
			if (chain
					&& (buf[ISO7816.OFFSET_INS] != chain_ins && p1p2 != chain_p1p2)) {
				resetChaining();
			...
			in_received = Util.arrayCopyNonAtomic(buf, ISO7816.OFFSET_CDATA,
					buffer, in_received, len);

			chain = true;
			chain_ins = buf[ISO7816.OFFSET_INS];
			chain_p1p2 = p1p2;

			ISOException.throwIt(ISO7816.SW_NO_ERROR);
		}

		if (chain && buf[ISO7816.OFFSET_INS] == chain_ins && p1p2 == chain_p1p2) {
			chain = false;
	...
	private void sendNext(APDU apdu, short status) {
		...
		if (out_left > max_length) {
			...
			if (out_left > max_length) {
				...
				status = (short) (ISO7816.SW_BYTES_REMAINING_00 | out_left);
			}
		}
		else {
			len = out_left;
	 ...
	 * ISO7816.SW_WARNING_STATE_UNCHANGED will be thrown and nothing will
	 * change.
	 */
